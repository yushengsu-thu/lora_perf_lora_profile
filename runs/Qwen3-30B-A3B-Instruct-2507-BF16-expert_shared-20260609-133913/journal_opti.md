...
| **TP3 / pinning / park** | per-rank trace + NUMA check | allreduce slowness = host-dispatch skew (compute balanced 4%; TP3 structural laggard 10/12, no-lora shows same at 2× vs LoRA 7.9×). NUMA pinning ALREADY active (workers cpus=0-71/72-143) → no cheap lever. PR #9 CLOSED; B (MoE-dispatch graph-traceable) is the only path, payoff ~5-13% e2e, multi-day → **PARKED** as documented future work; ship opt1/2/5 | — |
