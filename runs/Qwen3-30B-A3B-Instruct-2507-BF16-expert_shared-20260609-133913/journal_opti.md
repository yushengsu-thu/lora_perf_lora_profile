...
| **step8x (NEXT)** | **every LoRA layer as a piecewise split op** (qkv/row/column/replicated, mirror the MoE/embedding pattern in `piecewise_split.py`) — after that guards reduce to the size ladder | — | — |
