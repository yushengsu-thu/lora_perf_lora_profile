...
*(No further attention-LoRA work planned in the opt4–opt7 ladder: the cuBLAS/fuse opts above are
...
| **MoE-decomp extra** | 0 | **26.0** | **+26.0** | ~0 | in-MoE LoRA fold (bf16 = **NVFP4 sibling**, decomposed + missing-unfused-cubin wall → **NOT a port of FP8**; bf16-only CUTLASS grouped GEMM: **gather-prologue + SwiGLU·LoRA EVT epilogue**, no quant to fuse into) + fuse routing/align/topk/elem | • **align/sort/scatter +10.2** (`moe_align_block_size_small_batch` 6.7 + `moe_lora_merged::fused_align_scatter` 3.5, latter LoRA-specific) — ✅ opt1<br>• **fused_moe +7.2** (LoRA-Δ B-expand GEMM producing gate_upΔ — **NOT fold-absorbed**; the fold consumes a precomputed Δ, same as FP8/FP4's `gateUpLoraDeltaPtr`)<br>• **elem / copy / cast +3.9** (upcast / copy) — ✅ opt2<br>• **activation +3.2** (`moe::dev::activation`) — fold (opt7); prefill side-write killed first by **opt6** (`SGLANG_OPT_BF16_MOE_ACT_DROP_LORA_CAPTURE`)<br>• **topk / pack +3.0** (`_fused_virtual_topk_ids`) — ✅ opt2<br>• **permute +2.4** (`moe::dev::permute`) — fold (opt7 gather-prologue, `SGLANG_OPT_BF16_MOE_GEMM1_FOLD`; **180 µs/layer at prefill**, see prefill view) | 1. ✅ [opt1 — align/sort fusion: decode +11%](https://github.com/yushengsu-thu/sglang/commit/869882a3ab87ec3c1983f8808d382ef2aa1d0cea) · [results](opt1/)<br>2. ✅ [opt2 — topk/pack: decode +5.6%](opt2/) _(flag-only, no code commit)_<br>3. ✗ [opt3 — lean `_get_lora_info`: no clear win, kept default-on (harmless)](https://github.com/yushengsu-thu/sglang/commit/1536c6e4e65515f5ee7403c48b0726d55307d430) · [results](opt3/)<br>4. ✗ [opt4 — two-stream prefill A/B: NEGATIVE (prefill −8%), NOT adopted, keep 256](opt4/) _(flag-only experiment, zero code)_<br>5. ✅ [opt5 — prefill routing reuse: prefill +8~11%, align/sort 4×→2×](https://github.com/yushengsu-thu/sglang/commit/850faa87fbcc7d54210bc86866d2f9b3ecf4abce) · [results](opt5/)<br>6. ✗ [opt6 — drop act capture: mechanism verified (−34% act kernel) but sub-noise; default-OFF, plumbing reused by opt7](https://github.com/yushengsu-thu/sglang/commit/cf9d0e55e) + [default-off](https://github.com/yushengsu-thu/sglang/commit/f4971ea4e) · [results](opt6/)<br>7. ✦ [opt7 — in-MoE fold: ALL kernel gates PASS (−62% MoE prefill kernel time), acc at noise floor, but e2e +1~3% = host-bound absorbed; default-OFF, code ready on PR#4](opt7/OPT7.md) — commits [P0](https://github.com/yushengsu-thu/sglang/commit/1a82c2111)/[P1](https://github.com/yushengsu-thu/sglang/commit/7e1e69eb6)/[P2](https://github.com/yushengsu-thu/sglang/commit/f2247b5a8)/[P3](https://github.com/yushengsu-thu/sglang/commit/f7e5d5119)/[P4](https://github.com/yushengsu-thu/sglang/commit/dfa3493c9) |
| LoRA MoE shrink (routed experts) | 0 | 9.2 | +9.2 | ~0 | **NOT a fold target** (FP8/FP4 keep it separate) — two-stream overlap / cuBLAS; **opt7-② NOT implemented** (deferred: with opt7 e2e host-bound, the Δ-chain read-path change has no payoff until the host wall is addressed; flag name reserved `SGLANG_OPT_BF16_MOE_SHRINK_PERMUTED`) | | |
...
|---|---|---|
| standalone `permute` | **180** (largest single item; 3× GEMM1 itself) | fold **gather-prologue** (opt7) |
| standalone `activation` | 33 | fold EVT epilogue (opt7). **opt6 tried killing its redundant side-write first: mechanism verified (32.9→21.7 µs/call) but sub-noise on bench → default-off; its permuted-read+row-map plumbing feeds opt7** |
| Triton re-sort ×4 (`moe_align`+`count_and_sort`) | **119** (routing already computed by trtllm-gen; decode has opt1's fused path, prefill falls back) | ✅ **opt5 DONE 2026-06-11: prefill +7.4~8.2% (single) / +9.4~11.1% (two) @bs16/32/64, decode flat; align/sort 4×→2×/layer (−50%), −2688 launches.** Remaining 2×/layer = genuinely different sorts (shared-outer A vs per-expert B) — collapsing them = opt7's metadata pipeline. [commit 850faa87f](https://github.com/yushengsu-thu/sglang/commit/850faa87fbcc7d54210bc86866d2f9b3ecf4abce) · [opt5/](opt5/) |
| LoRA-Δ GEMMs (shrink/fused_moe/expand) | 226 | stays; **opt7-②** feeds the shrink from the base path's `permuted_hidden_bf16` (contiguous expert-grouped reads instead of its own gather) + two-stream overlap |
...
- `permuted_hidden_bf16` (expert-grouped, padded — already grouped-GEMM layout) is directly
  readable by the gate_up LoRA shrink (**opt7-②**). fp8 never materializes it; nvfp4 materializes
...
- `activated_bf16` is **bitwise the same values** as the `activation_lora_input` side-capture
  ...
  which is *why* fp8/fp4 must keep the capture) → bf16 can drop it (**opt6**).
...
Execution order: **opt4 ✗ → opt5 ✅ (+8~11% prefill) → opt6 ✗ (infra) → opt7-step0 ✅ → opt7 ✦ (kernels ✅, e2e host-bound). LADDER COMPLETE — next lever is HOST-side (prefill graphing/launch batching), not kernel fusion.**
...
|---|---|---|---|---|
| **opt4** | two-stream-at-prefill A/B (`SGLANG_TWO_STREAM_MAX_TOKENS` 256→8192) | existing env (common) | **zero code** (flag A/B) | ✗ **DONE 2026-06-11, NEGATIVE — prefill −8~9% @bs16/32/64 (noise floor ±2%), decode flat; keep 256.** Confirms prefill bottleneck ≠ serialization → remove work (opt5), don't rearrange it. See `opt4/` |
...
1. **in-MoE LoRA fold** — biggest structural win. Corrected accounting (2026-06-11): the fold-only
   ...
   (ref §5/§6 + journal opt7). De-risked by **prefill-only dispatch + dual-layout gemm1 weights**
   (see the future-work ladder). Do **opt4** (two-stream-prefill A/B, done), **opt5/opt6**
   (routing-metadata + shared-buffer reuse) and the **opt7-step0 unfused-cubin probe** first —
